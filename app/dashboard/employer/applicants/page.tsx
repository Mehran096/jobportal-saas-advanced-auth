...
function ApplicantsContent() {
  ...
  const [loadingAction, setLoadingAction] = useState<FinalStatus | null>(null); // <-- ADDED
  ...
  const handleUpdateStatus = async (appId: string, newStatus: FinalStatus) => {
    ...
    setLoadingAction(newStatus); // <-- START LOADING
    try {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-6xl mx-auto p-4 sm:p-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold">{jobId? "Job Applicants" : "All Applicants"}</h1>
        ...
        <div className="flex items-center gap-2 mb-6 p-3 bg-white rounded-xl shadow-sm border overflow-x-auto">
          ...
          {(["all", "pending", "shortlisted", "accepted", "rejected"] as Status[]).map((status) => (
            <button key={status} onClick={() => setFilter(status)} className={`px-4 py-2 rounded-lg text-sm font-medium capitalize whitespace-nowrap ${filter === status? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
        ...
        {filteredApps.length === 0? (
          ...
          <div className="space-y-4">
            {filteredApps.map((app) => {
              ...
              if (!s) return null;
              return (
                <div key={app._id} onClick={() => setSelectedApp(app)} className="bg-white p-5 rounded-xl border hover:shadow-md cursor-pointer">
                  <div className="flex gap-4">
                    <Image src={s.profileImage || `https://ui-avatars.com/api/?name=${s.firstName}+${s.lastName}&background=random`} width={56} height={56} unoptimized alt="avatar" className="w-14 h-14 rounded-full object-cover" />
                    <div className="flex-1">
                      <div className="flex justify-between">
                        <div>
                          ...
                          <div className="flex gap-1 mt-2 flex-wrap">{s.skills?.slice(0, 4).map((sk) => <span key={sk} className="bg-blue-50 text-blue-700 text-[11px] px-2 py-0.5 rounded-full">{sk}</span>)}</div>
                        </div>
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold capitalize h-fit ${statusColors[app.status]}`}>{app.status}</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="p-6">
                ...
                <div className="flex gap-4 mb-5">
                  <Image src={selectedApp.snapshot.profileImage || `https://ui-avatars.com/api/?name=${selectedApp.snapshot.firstName}`} width={80} height={80} unoptimized alt="profile" className="w-20 h-20 rounded-full object-cover" />
                  <div>
                    <h3 className="text-xl font-bold">{selectedApp.snapshot.firstName} {selectedApp.snapshot.lastName}</h3>
                    <p className="text-blue-600 text-sm">{selectedApp.snapshot.headline}</p>
                    <p className="text-gray-600 text-sm mt-1">{selectedApp.snapshot.bio || "No bio"}</p>
                    <div className="text-sm mt-2 space-y-1 text-gray-600">
                      <p className="flex gap-2"><Mail size={14} /> {selectedApp.snapshot.email}</p>
                      <p className="flex gap-2"><Phone size={14} /> {selectedApp.snapshot.phone}</p>
                      <p className="flex gap-2"><MapPin size={14} /> {selectedApp.snapshot.location}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">{selectedApp.snapshot.skills?.map((sk) => <span key={sk} className="bg-gray-100 px-3 py-1 rounded-full text-sm">{sk}</span>)}</div>
                </div>

                <div className="bg-blue-50 p-4 rounded-xl flex justify-between items-center">
                  <span className="flex items-center gap-2 font-medium"><FileText size={18} className="text-blue-600" /> {selectedApp.snapshot.resumeName || "Resume.pdf"}</span>
                  <div className="flex gap-2">
                    <a href={selectedApp.snapshot.resumeUrl} target="_blank" className="bg-white border px-4 py-2 rounded-lg text-sm flex items-center gap-1"><ExternalLink size={14} /> View</a>
                    <a href={selectedApp.snapshot.resumeUrl} download className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm flex items-center gap-1"><Download size={14} /> Download</a>
                  </div>
                </div>
                ...
                {FINAL_STATUSES.includes(selectedApp.status)? (
                  <div className="mt-6 p-4 bg-gray-50 border border-dashed rounded-xl text-center">
                    ...
                    <span className={`mt-2 inline-block px-4 py-1 rounded-full text-sm font-bold capitalize ${statusColors[selectedApp.status]}`}>
                      {selectedApp.status}
                    </span>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-6">
                    <button
                      ...
                      className="bg-purple-600 text-white py-2.5 rounded-xl text-sm hover:bg-purple-700 disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {loadingAction === "shortlisted" && <Loader2 size={14} className="animate-spin" />}
                      {loadingAction === "shortlisted"? "Shortlisting..." : "Shortlist"}
                    </button>
                    <button
                      onClick={() => handleUpdateStatus(selectedApp._id, "accepted")}
                      disabled={!!loadingAction}
                      className="bg-green-600 text-white py-2.5 rounded-xl text-sm hover:bg-green-700 disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {loadingAction === "accepted" && <Loader2 size={14} className="animate-spin" />}
                      {loadingAction === "accepted"? "Accepting..." : "Accept"}
                    </button>
                    <button
                      onClick={() => handleUpdateStatus(selectedApp._id, "rejected")}
                      disabled={!!loadingAction}
                      className="bg-red-600 text-white py-2.5 rounded-xl text-sm hover:bg-red-700 disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {loadingAction === "rejected" && <Loader2 size={14} className="animate-spin" />}
