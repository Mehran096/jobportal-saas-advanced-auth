...
function PeopleAlsoSearch({ currentSearch, allJobs, onSelect }: { currentSearch: string; allJobs: Job[]; onSelect: (q: string) => void }) {
  ...
  return (
    <div className="mt-8 bg-white border border-gray-100 rounded-2xl p-5 md:p-6 shadow-sm">
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto mb-16 p-4 sm:p-6 pb-24 md:pb-6">
        <h1 className="text-2xl sm:text-3xl font-bold mb-1 sm:mb-2">Find Your Dream Job</h1>
        <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">Browse {jobs.length} open positions — {savedJobIds.length} saved</p>

        <div className="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100 mb-4 flex flex-col sm:flex-row gap-3">
        ...
        {/* ✅ POLISHED: Filters in one line + horizontal scroll on mobile */}
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm mb-6 sm:mb-8">
          <div className="w-full overflow-x-auto scrollbar-hide">
            <div className="flex items-center gap-2 sm:gap-3 p-3 flex-nowrap">
              <select value={datePosted} onChange={(e) => { setDatePosted(e.target.value); setCurrentPage(1) }} className="shrink-0 px-4 py-2.5 border border-gray-200 rounded-full text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none font-medium">
              ...
              <select value={jobType} onChange={(e) => { setJobType(e.target.value); setCurrentPage(1) }} className="shrink-0 px-4 py-2.5 border border-gray-200 rounded-full text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none font-medium">
              ...
              <select value={minSalary} onChange={(e) => { setMinSalary(e.target.value); setCurrentPage(1) }} className="shrink-0 px-4 py-2.5 border border-gray-200 rounded-full text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none font-medium">
              ...
              {(jobType || minSalary || datePosted) && (
                <button onClick={clearFilters} className="shrink-0 ml-2 flex items-center gap-1.5 px-4 py-2.5 text-sm text-blue-600 font-semibold hover:bg-blue-50 rounded-full transition whitespace-nowrap">
        ...
        {displayJobs.length === 0? (
          <div className="text-center py-16 sm:py-20 bg-white rounded-xl shadow-sm border">
            <Briefcase className="mx-auto text-gray-400 mb-4" size={48} />
            <p className="text-gray-500 px-4">No jobs found for {search || jobType || "your filters"}. Try different keywords.</p>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {displayJobs.map((job) => {
                ...
                return (
                  <div key={job._id} className="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition flex flex-col group">
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <h3 className="text-[17px] sm:text-xl font-semibold group-hover:text-blue-600 transition line-clamp-1">{job.title}</h3>
                        <button onClick={() => handleToggleSave(job._id)} className={`shrink-0 p-2 rounded-full border transition ${isSaved? "bg-blue-50 border-blue-200 text-blue-600" : "bg-white hover:bg-gray-50 text-gray-400"}`}>
                          {isSaved? <BookmarkCheck size={18} /> : <Bookmark size={18} />}
                        </button>
                      </div>
                      <p className="text-gray-700 font-medium mb-1.5 text-sm sm:text-base">{job.company}</p>
                      ...
                      <div className="space-y-2 text-sm text-gray-500 mb-4">
                        <div className="flex items-center gap-2"><MapPin size={14} className="shrink-0" /> <span className="truncate">{job.location}</span></div>
                    ...
                    <div>
                      <p className="text-[11px] text-gray-400 mb-3">Posted: {job.createdAt? new Date(job.createdAt).toLocaleDateString() : "N/A"}</p>
                      {alreadyApplied? <div className="w-full py-2.5 rounded-lg font-semibold bg-gray-100 text-gray-500 text-center text-sm">✓ Applied</div> : <Link href={`/dashboard/jobs/${job._id}`} className="w-full py-2.5 rounded-lg font-semibold bg-blue-600 hover:bg-blue-700 text-white text-center block transition text-sm">View Details</Link>}
                    </div>
                  </div>
                );
              })}
